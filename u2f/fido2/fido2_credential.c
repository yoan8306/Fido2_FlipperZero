...
Fido2CredentialStore* fido2_credential_store_alloc() {
    Fido2CredentialStore* store = malloc(sizeof(Fido2CredentialStore));
    memset(store, 0, sizeof(Fido2CredentialStore));
...
bool fido2_credential_sign(
    ...
    mbedtls_mpi_write_binary(&r, r_bytes + (33 - r_len), r_len);
    mbedtls_mpi_write_binary(&s, s_bytes + (33 - s_len), s_len);

    // Pad to 32 bytes
    if(r_len < 32) {
        memmove(r_bytes + 32 - r_len, r_bytes + 33 - r_len, r_len);
        memset(r_bytes, 0, 32 - r_len);
        r_len = 32;
    }
    if(s_len < 32) {
        memmove(s_bytes + 32 - s_len, s_bytes + 33 - s_len, s_len);
        memset(s_bytes, 0, 32 - s_len);
        s_len = 32;
    }
    ...
    memcpy(signature + offset, r_bytes + (33 - r_len), r_len);
    offset += r_len;
    signature[offset++] = 0x02; // INTEGER
    signature[offset++] = s_len;
    memcpy(signature + offset, s_bytes + (33 - s_len), s_len);
    offset += s_len;

...
static int32_t fido2_hid_worker(void* context) {
    ...
    // Save current USB config and switch to HID U2F mode
    ...
    while(1) {
        ...
        if(flags & WorkerEvtConnect) {
            FURI_LOG_I(WORKER_TAG, "Device connected");
        }

        if(flags & WorkerEvtDisconnect) {
            FURI_LOG_I(WORKER_TAG, "Device disconnected");
        }
...
Fido2Hid* fido2_hid_start(Fido2Ctap* ctap) {
    ...
    FURI_LOG_I(TAG, "FIDO2 HID started");
    return fido2_hid;
}
